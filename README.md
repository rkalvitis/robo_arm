...
| `_lens_xyz` | ultra-wide lens of the iPhone 15 Pro, measured from `phone_mount_conf3.stl`: `-0.0225,-0.0431,0.1170` (m) — the **top-left ring** of the bump, physically verified 2026-08-25 (the bottom-left ring at `-0.0225,-0.0239,0.1170` was wrongly used before; the 19.2 mm ring spacing caused the arc drift) | Phone-lens position in the **flange frame** (`panda_link8`). The object is placed `radius` from the **lens along the camera axis**, and the arc keeps the lens (not the flange) at `radius`. `_lens_xyz:=''` falls back to the legacy scalar `_camera_offset` behavior. **If the arc drifts off the object as the angle grows, these constants are wrong for the physical phone — run `calibrate_lens.py` (below) and use its output** |
